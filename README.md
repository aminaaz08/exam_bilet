...
**Билет 2. «Переменные и константы в языке Go»**
---

Язык Go сочетает простоту синтаксиса с мощной системой типов и строгим управлением памятью. Переменные и константы — это фундаментальные элементы любой программы, и Go предоставляет чёткие, безопасные и эффективные механизмы для их объявления, инициализации и использования. Рассмотрим все аспекты подробно.
...
### **1. Создание**
Константы в Go создаются с помощью ключевого слова `const`. Они **не могут изменяться** после объявления.

### **2. Вывод типа: нетипизированные и типизированные константы**

- **Нетипизированные константы** — не имеют фиксированного типа до момента использования:
  ```go
  const Pi = 3.14159 // нетипизированная
  var x float64 = Pi // здесь Pi становится float64
  var y int = Pi     // ошибка — нельзя присвоить float → int
  ```

- **Типизированные константы** — явно указывают тип:
  ```go
  const Max int = 100
  ```

> Нетипизированные константы обеспечивают **гибкость**: одну и ту же константу можно использовать с разными числовыми типами, если значение совместимо.

### **3. Что может быть константой?**
Только **константные выражения**, вычисляемые на этапе компиляции:
- Числа, строки, булевы значения,
- Арифметические и логические операции над константами,
- Вызовы встроенных функций (`len`, `cap` — но только для константных массивов/срезов).

> **Нельзя**: вызовы пользовательских функций, обращение к переменным, чтение из файлов и т.д.

### **4. Способы объявления**
...
- **Локальная константа** — внутри функции (редко используется, но допустимо):
  ```go
  func f() {
      const local = 42
  }
...
- **Константа уровня пакета** — вне функций, в начале файла:
  ```go
  const Version = "1.0"
  ```
  Такие константы видны во всех файлах пакета.

### **6. Влияние порядка объявления**
Go позволяет использовать **ранее объявленные константы** в последующих:
```go
const (
    A = 1
    B = A + 1 // OK
...
```
Но нельзя ссылаться на константу, объявленную ниже.

### **7. Использование `_` как имени константы**
Идентификатор `_` — это **пустая переменная**, которая отбрасывает значение.  
Для констант он **не имеет практического смысла**, так как константы не вычисляются во время выполнения. Однако синтаксически допустимо:
```go
const _ = 42 // компилируется, но бесполезно
```
> Чаще `_` используется для переменных (например, при множественном возврате: `_, err := someFunc()`).
...
- Условиях (`if`, `switch`),
- Размерах массивов: `var arr [Max]int`,
- Аргументах функций (если тип совместим).

#### **Нельзя использовать в:**
- Контекстах, требующих **адреса** (см. ниже),
- Выражениях, требующих **времени выполнения**.

### **9. Адресуемость констант**
**Константы не имеют адреса в памяти**. Нельзя взять адрес константы:
```go
const X = 10
p := &X // ОШИБКА: cannot take address of X
```
Причина: константы — это **компиляционные сущности**, они могут быть встроены напрямую в инструкции процессора.
...
### **1. Создание**
Переменные в Go создаются через `var` или краткую форму `:=`.
...
- **Переменная уровня пакета** — вне функций:
  ```go
  var Counter int = 0
  ```
  Видна во всех файлах пакета. Если имя начинается с заглавной буквы — экспортируется в другие пакеты.

### **4. Влияние порядка объявления**
Для переменных уровня пакета возможна **зависимость от порядка**:
```go
var (
    A = 1
    B = A + 1 // OK
...
```
Но циклические зависимости запрещены.
...
- **Ограничения**:
  ...
  - Хотя бы одна переменная должна быть **новой** (иначе ошибка компиляции).

### **6. Использование `_` как имени переменной**
Отбрасывает значение:
```go
_, err := os.Open("file.txt")
if err != nil { /* обработка */ }
```
Полезно, когда часть возвращаемых значений не нужна.

### **7. Инициализация**

#### **Неинициализированная переменная**
Если переменная не инициализирована явно, ей присваивается **нулевое значение** типа:
- `int` → `0`
- `bool` → `false`
- `string` → `""`
- указатель, слайс, карта, канал → `nil`

#### **Почему именно так?**
Go гарантирует, что **все переменные инициализированы**, чтобы избежать неопределённого поведения (в отличие от C++).

#### **Назначение инициализации**
- Предотвращает использование «мусора»,
- Упрощает отладку и повышает безопасность.

### **8. Определение vs Объявление**
В Go **нет различия** между объявлением и определением — каждое объявление **выделяет память**. Это упрощает модель по сравнению с C++.

### **9. Способы выделения памяти**

#### **Стек**
- Все локальные переменные (включая структуры) размещаются на стеке.
- Автоматически уничтожаются при выходе из функции.

#### **Куча**
- Объекты, на которые есть ссылки за пределами функции (например, возвращаемые значения),
- Слайсы, карты, каналы, указатели — всегда создаются в куче (или частично),
- Управление памятью осуществляется **сборщиком мусора (GC)**.

> Программист **не вызывает `free`** — Go делает это автоматически.

### **10. Использование: доступ к переменной**
- По имени: `x = 10`
- Через указатель: `*p = 20`
- Через составные типы: `arr[0]`, `m["key"]`

### **11. Связывание идентификатора**

#### **Статическое (лексическое) связывание**
Имя связывается с переменной **на этапе компиляции** — основной механизм в Go.

#### **Динамическое связывание**
**Не применяется к переменным** в Go (в отличие от виртуальных методов в C++).

#### **Переменная в другом `.go` файле**
- **Того же пакета**: доступна напрямую, если объявлена на уровне пакета.
- **Другого пакета**: только если имя начинается с **заглавной буквы** (экспортируемая):
  ```go
  // pkg/utils.go
  var PublicVar = 42
...
- **Блочная**: внутри `{}` (включая `if`, `for`, функции),
- **Пакетная**: вне функций,
- **Универсальная**: встроенные идентификаторы (`true`, `len`, `panic`).

### **13. Время жизни переменной**

- **Локальные переменные**: живут до конца блока.
- **Переменные уровня пакета**: живут всё время выполнения программы.
- **Слайсы и карты**: данные хранятся в куче, время жизни определяется GC.

#### **Значение неинициализированной переменной**
— нулевое значение (см. выше).

### **14. Словарь (карта)**
Карта (`map`) — встроенный тип для хранения пар «ключ → значение»:
```go
var m map[string]int          // nil-карта
m = make(map[string]int)      // выделение памяти
m["age"] = 25
```
- Неинициализированная карта — `nil`, запись в неё вызывает панику.
- Карта всегда передаётся **по ссылке** (на самом деле — по значению заголовка, но данные общие).

---

### **Заключение**

Переменные и константы в Go спроектированы с акцентом на **безопасность**, **простоту** и **производительность**. Отсутствие неинициализированных значений, чёткие правила видимости, автоматическое управление памятью и мощная система экспорта делают Go удобным для написания надёжного кода. Понимание этих концепций — залог успешной разработки и высокой оценки на экзамене.
